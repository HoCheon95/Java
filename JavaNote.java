...
19) 추상메서드
 ...
 - 반드시 자식클래스에서 구현해야 된다 라는 강제 규칙을 만들기 위해서 사용
...
22. 제네릭
 - <제네릭 타입>은 기본타입으로 지정할 수 없고 참조 레퍼런스 타입으로 지정해야 한다. <제네릭 타입>은
   자바 5버전에서(jdk 1.5)에서 추가되었다.
   
 - 제네릭 타입을 지정하면 지정한 자료형으로만 저장 가능하다.
 
 - 제네릭 타입을 지정하지 않으면 불필요한 레퍼런스 간의 형변환인 업캐스팅과 다운캐스팅을 해야 한다. 그만큼
   프로그램 실행속도가 떨어지고 코드가 복잡해 져서 가독성이 안 좋아진다. 그리고 원하는 자료 추출도 어려워서
   자료의 안정성도 떨어진다.
   
 - 앞 부분 <String, Integer> 제네릭 타입만 봐도 뒤부분 제네릭 타입이 무엇인지 추론이 가능하다.
   그래서 자바 7버전부터는 뒷 부분 제네릭 타입 지정은 다음과 같이 생략되었다.
   Map<String, Integer> hm = new HashMap<>();
   
 - 제네릭 타입간의 레퍼런스간의 형변환은 허용되지 않는다. 안되는 이유는 제네릭 타입들 간에는 상속 관계가
   성립되지 않기 때문이다. 만약 허용한다면 '자료의 안정성이 깨질 수 있기 때문에' 자바 컴파일러가 이를 막는다.
   따라서 자바 컴파일러는 이를 엄격히 제한하며 컴파일 단계에서 오류를 발생시킨다.
  
 - 리스트 인터페이스에 값을 저장할 때는 '기본 타입은 자동으로 래퍼 클래스가되고' ('Object로 업캐스팅'이되어 저장된다.)
   이후 리스트에서 값을 꺼내어 '구체 타입의 기능을 사용'하려면
   '반드시 다운캐스팅이 필요'하며,
   이때 실제 저장된 타입과 캐스팅 타입이 다르면 런타임 에러가 발생한다.
   이러한 문제를 사전에 차단하기 위해
   제네릭을 사용하여 '리스트에 저장할 타입을 하나로 제한'한다.
...
23. 제네릭 클래스
 Day16_java/src/ListEx12.java
 - class GenericClass<T>{// <T> 정해지지 않은 가상의 제네릭 T타입

24. 제네릭 와일드카드
	(1) 첫번째 제네릭 와일드 카드 문법형식)
	Day16_java/src/ListEx14.java
	...
	(2) 두번쨰 제네릭 와일드 카드 문법형식)
	Day16_java/src/ListEx15.java
	...
	<?> 모든 객체타입 배치가 가능하다.
		...
		static void printList(List<?> list){
			...
			for(Object obj:list) {
				/*
				 * list의 제네릭 타입이 정해져 있지 않아서 요소의 구체 타입을 알수 없기 때문에
	...
	(3) 세번째 제네릭 와일드 카드 문법형식)
	Day16_java/src/ListEx16.java
...
25. 예외처리
	(1) try ~ catch 문으로 예외 처리)
	Day17_java/src/TryEx01.java
	
		try{
			정상구문이 실행되다가 예외가 발생하면 아래 문장을 수행하지 않고 예외 처리 해당 catch 블록으로
			이동해서 예외를 처리한다.
			...
		}catch(예외처리클래스 매개변수){
			예외처리 코드;
	...
	(2) 하나의 try 문에 여러개의 catch 블록문이 ({}) 오는 경우)
	Day17_java/src/TryEx02.java
		...
		try{
			...
			이유는 자바 런타임이 예외 발생시 catch 블록문을 위에서부터 순차적으로 검사하면서, 가장 구체적이고
	...
	Day17_java/src/TryEx04.java
		
		참조변수만 선언하고 null 이 대입된 상태에서 해당 클래스의 소속 멤버변수등에 접근하면 완전한 객체주소가
		저장안된 상태이기 때문에 NullPointerException 예외 오류를 발생시킨다.
	
	(5) ArrayIndexOutOfBoundsException
	Day17_java/src/TryEx05.java
		
		배열주소 인덱스 번호 범위를 벗어나면 ArrayIndexOutOfBoundsException 예외오류를 발생시킨다.
	
	(6) throw and throws
	Day17_java/src/TryEx06.java
